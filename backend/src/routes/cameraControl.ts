...
import { uuidParamSchema, cameraControlSchema } from '../schemas.js';
...
cameraControl.post('/:id/start', zValidator('param', uuidParamSchema), async (c) => {
  try {
...
cameraControl.post('/:id/stop', zValidator('param', uuidParamSchema), async (c) => {
  try {
    const user = c.get('user');
    const { id: cameraId } = c.req.valid('param');
